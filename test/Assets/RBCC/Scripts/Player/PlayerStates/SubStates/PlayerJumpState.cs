...
namespace RBCC.Scripts.Player.PlayerStates.SubStates
{
    public class PlayerJumpState : PlayerBaseState
    {
        ...
        private void HandleJumpTime()
        {
            ...
            if (Ctx.IsJumpingUp)
            {
                ...
                if (_jumpTime > _jumpHoldTime ||
                    MathUtils.LessThanApprox(Vector3.Dot(Vector3.Project(Ctx.Rb.linearVelocity, Ctx.UpDirection), Ctx.UpDirection), 0f)) // Going downwards
                {
        ...
        private void CancelJump(InputAction.CallbackContext context)
        {
            if (context.canceled)
            {
                Vector3 cancelForce = jumpData.jumpCancelRate * Ctx.Rb.mass * -Vector3.Project(Ctx.Rb.linearVelocity, Ctx.groundRayDir);
